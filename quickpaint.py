...
class QuickPaintOperations():
    ...
    def optimizeObjects():
        """
        This function merges all touching objects of the same type. We don't want huge files for level data. Nor do we don't want an island to be completely made up of 1x1 objects. And we most definately don't want objects more than 1x1 to repeat only the first tile in them.
        """
        optimize_memory = []
        for ln in range(len(globals.Area.layers)):
            ...
            while len(list(filter(lambda i:i.layer == ln,QuickPaintOperations.object_optimize_database))) > 0:
                ...
                obj = min(list(filter(lambda i:i.layer == ln,QuickPaintOperations.object_optimize_database)), key=lambda i: sqrt(i.objx**2+i.objy**2))
                w = 1024
                cobj = obj
                ...
                for y in range(512):
                    ...
                    for x in range(w):
                        ...
                        atd_archive.append((x,y,None if not hasattr(cobj, 'modifiedForSize') else cobj.modifiedForSize, None if not hasattr(cobj, 'autoTileType') else cobj.autoTileType))
                        if cobj is None or cobj in objects_inside_optimize_boxes or cobj.tileset != obj.tileset or cobj.type != obj.type:
                            w = x
                            break
                ...
                else: lets_use_these_dimensions = [0,0,[]]
                if lets_use_these_dimensions[0] * lets_use_these_dimensions[1] == 0:
                    QuickPaintOperations.object_optimize_database.remove(obj)
                    continue
                calculated_rectangle = (obj.objx, obj.objy, lets_use_these_dimensions[0], lets_use_these_dimensions[1], obj, list(set(atd_archive)))
    ...
    def prePaintObject(ln, layer, x, y, z ):
        ...
        """
        if not hasattr(QuickPaintOperations, 'prePaintedObjects'): QuickPaintOperations.prePaintedObjects = {}
        if not ('%i_%i'%(x, y) in QuickPaintOperations.prePaintedObjects):
            QuickPaintOperations.prePaintedObjects['%i_%i'%(x, y)] = {'ln': ln, 'layer': layer, 'x':x, 'y':y, 'z':z , 'r': int(rand()*127)+128, 'g': int(rand()*127)+128, 'b': int(rand()*127)+128}
    ...
    def PaintFromPrePaintedObjects():
        ...
        if hasattr(QuickPaintOperations, 'prePaintedObjects'):
            for ppobj in QuickPaintOperations.prePaintedObjects:
                ...
                QuickPaintOperations.AddObj(QuickPaintOperations.prePaintedObjects[ppobj]['ln'], QuickPaintOperations.prePaintedObjects[ppobj]['layer'], QuickPaintOperations.prePaintedObjects[ppobj]['x'], QuickPaintOperations.prePaintedObjects[ppobj]['y'], QuickPaintOperations.prePaintedObjects[ppobj]['z'])
                QuickPaintOperations.AddObj(QuickPaintOperations.prePaintedObjects[ppobj]['ln'], QuickPaintOperations.prePaintedObjects[ppobj]['layer'], QuickPaintOperations.prePaintedObjects[ppobj]['x'], QuickPaintOperations.prePaintedObjects[ppobj]['y'], QuickPaintOperations.prePaintedObjects[ppobj]['z'])
    ...
    def preEraseObject(ln, layer, x, y):
        ...
        if not hasattr(QuickPaintOperations, 'prePaintedObjects'): QuickPaintOperations.prePaintedObjects = {}
        if not ('%i_%i'%(x, y) in QuickPaintOperations.prePaintedObjects):
            QuickPaintOperations.prePaintedObjects['%i_%i'%(x, y)] = {'ln': ln, 'layer': layer, 'x':x, 'y':y, 'z':0 , 'r': int(rand()*127), 'g': int(rand()*127), 'b': int(rand()*127)}
    @staticmethod
    def EraseFromPreErasedObjects():
        ...
        if hasattr(QuickPaintOperations, 'prePaintedObjects'):
            for ppobj in QuickPaintOperations.prePaintedObjects:
                #We do this action twice to make sure there is no trailing.
                QuickPaintOperations.EraseObj(QuickPaintOperations.prePaintedObjects[ppobj]['ln'], QuickPaintOperations.prePaintedObjects[ppobj]['layer'], QuickPaintOperations.prePaintedObjects[ppobj]['x'], QuickPaintOperations.prePaintedObjects[ppobj]['y'])
                QuickPaintOperations.EraseObj(QuickPaintOperations.prePaintedObjects[ppobj]['ln'], QuickPaintOperations.prePaintedObjects[ppobj]['layer'], QuickPaintOperations.prePaintedObjects[ppobj]['x'], QuickPaintOperations.prePaintedObjects[ppobj]['y'])
        QuickPaintOperations.prePaintedObjects.clear()
        globals.mainWindow.scene.invalidate()
    @staticmethod
    def AddObj(ln, layer, x, y, z):
        ...
        if globals.mainWindow.quickPaint is not None and globals.mainWindow.quickPaint.scene is not None:
            ...
            if qp_data['base']['i'] is not None and qp_data['base']['ts'] is not None and qp_data['base']['t'] is not None:
                ...
                while objBehindThisOne is not None:
                    ...
                    if objBehindThisOne in QuickPaintOperations.object_optimize_database: QuickPaintOperations.object_optimize_database.remove(objBehindThisOne)
                ...
                if obj not in QuickPaintOperations.object_optimize_database: QuickPaintOperations.object_optimize_database.append(obj)
                mw.scene.addItem(obj)
                connected_objects = []
                #This next function has to repeated multiple times at multiple places in and around this object. Otherwise, you will leave artifacts when painting with bigger objects.
        ...
        if globals.mainWindow.quickPaint is not None and globals.mainWindow.quickPaint.scene is not None:
            ...
            if qp_data['base']['i'] is not None and qp_data['base']['ts'] is not None and qp_data['base']['t'] is not None:
                ...
                if obj is not None:
                    ...
                    obj.delete()
                    obj.setSelected(False)
                    if obj in QuickPaintOperations.object_optimize_database: QuickPaintOperations.object_optimize_database.remove(obj)
    ...
    def searchObj( layer, x, y):
        ...
        """
        if QuickPaintOperations.object_search_database.get((x, y, layer)) is None: return None
        if len(QuickPaintOperations.object_search_database[(x, y, layer)]) <= 0: return None
    ...
    def sliceObjRange( posList):
        ...
        for pos in map(lambda i : (posList[i]['x'], posList[i]['y'], posList[i]['ln']), posList):
            ...
            #Way to go Robert!
            #Actually Bobj stands for base object, because these are the objects we start with as they lie on the positions where the user has painted over on the widget.
            # - John10v10
        ...
        while not no_more_found:
            ...
            if len(preobjlist) <= 0: no_more_found = True
            else: objlist = preobjlist
    ...
    def sliceObj( obj, px=None, py=None):
        ...
        """
        if obj is None or (obj.width == 1 and obj.height == 1):
            if obj not in QuickPaintOperations.object_optimize_database and obj is not None: QuickPaintOperations.object_optimize_database.append(obj)
            return obj
        out_obj = None
        mw = globals.mainWindow
        objx = obj.objx
        ...
        t = obj.original_type
        atd_archive = []
        if hasattr(obj, 'atd_archive') and obj.atd_archive is not None: atd_archive = obj.atd_archive
        skip = []
        x = 0
        y = 0
        if obj.objdata is not None:
            for row in obj.objdata:
                x = 0
                for tile in row:
                    if tile == -1: skip.append((x, y))
                    x+=1
                y+=1
        ...
        for y in range(h):
            for x in range(w):
                ...
                if len(layer) == 0:
                    ...
                    z = layer[-1].zValue() + 1
                objBehindThisOne = QuickPaintOperations.sliceObj(QuickPaintOperations.searchObj(ln,x+objx, y+objy), x+objx, y+objy)
                ...
                layer.append(sobj)
                if sobj not in QuickPaintOperations.object_optimize_database: QuickPaintOperations.object_optimize_database.append(sobj)
    ...
    def autoTileObj(layer, obj):
        """
        Automatically picks the tile that best fits its position. It's a big process, but I hope it works well enough for Miyamoto users.
        """
        if globals.mainWindow.quickPaint is not None and globals.mainWindow.quickPaint.scene is not None and obj is not None:
            qpscn = globals.mainWindow.quickPaint.scene
            qp_data = qpscn.object_database
            ...
            if QuickPaintOperations.isObjectInQPwidget(obj):
                ...
                if typeToTile is not None and qp_data.get(typeToTile) is not None and qp_data[typeToTile]['i'] is not None:
                        ...
                        for y in range(qp_data['topLeftCorner']['oh']):
                            for x in range(qp_data['topLeftCorner']['ow']):
                                if x > 0 or y > 0:
                                    ...
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'topLeftCorner')  and not (hasattr(sobj, 'modifiedForSize') and sobj.modifiedForSize&0x100 == 0x100):
                    ...
                    if qp_data['topRightCorner']['i'] is not None:
                        for y in range(qp_data['topRightCorner']['oh']):
                            for x in range(qp_data['topRightCorner']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx-x, obj.objy+y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'topRightCorner') and not (hasattr(sobj, 'modifiedForSize') and sobj.modifiedForSize&0x200 == 0x200):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                    ...
                    if qp_data['bottomLeftCorner']['i'] is not None:
                        for y in range(qp_data['bottomLeftCorner']['oh']):
                            for x in range(qp_data['bottomLeftCorner']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx+x, obj.objy-y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'bottomLeftCorner')  and not (hasattr(sobj, 'modifiedForSize') and sobj.modifiedForSize&0x400 == 0x400):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                    ...
                    if qp_data['bottomRightCorner']['i'] is not None:
                        for y in range(qp_data['bottomRightCorner']['oh']):
                            for x in range(qp_data['bottomRightCorner']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx-x, obj.objy-y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'bottomRightCorner')  and not (hasattr(sobj, 'modifiedForSize') and sobj.modifiedForSize&0x800 == 0x800):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                                        obj.modifiedForSize |= 0x800
                    if qp_data['topLeft']['i'] is not None and QuickPaintOperations.countUpLeft(layer, obj, qp_data['topLeft']['ow'],qp_data['topLeft']['oh']):
                        for y in range(qp_data['topLeft']['oh']):
                            for x in range(qp_data['topLeft']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx-x, obj.objy-y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'topLeft') and not (hasattr(sobj, 'modifiedForSize') and sobj.modifiedForSize&0x01 == 0x1):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                                        obj.modifiedForSize |= 0x01
                            
                    if qp_data['topRight']['i'] is not None and QuickPaintOperations.countUpRight(layer, obj, qp_data['topRight']['ow'],qp_data['topRight']['oh']):
                        for y in range(qp_data['topRight']['oh']):
                            for x in range(qp_data['topRight']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx+x, obj.objy-y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'topRight') and not (hasattr(sobj, 'modifiedForSize')  and sobj.modifiedForSize&0x02 == 0x2):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                                        obj.modifiedForSize |= 0x02
                    if qp_data['bottomLeft']['i'] is not None and QuickPaintOperations.countDownLeft(layer, obj, qp_data['bottomLeft']['ow'],qp_data['bottomLeft']['oh']):
                        for y in range(qp_data['bottomLeft']['oh']):
                            for x in range(qp_data['bottomLeft']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx-x, obj.objy+y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and  (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'bottomLeft') and not (hasattr(sobj, 'modifiedForSize') and sobj.modifiedForSize&0x04 == 0x4):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                                        obj.modifiedForSize |= 0x04
                    if qp_data['bottomRight']['i'] is not None and QuickPaintOperations.countDownRight(layer, obj, qp_data['bottomRight']['ow'],qp_data['bottomRight']['oh']):
                        for y in range(qp_data['bottomRight']['oh']):
                            for x in range(qp_data['bottomRight']['ow']):
                                if x > 0 or y > 0:
                                    sobj = QuickPaintOperations.searchObj(layer, obj.objx+x, obj.objy+y)
                                    if sobj is not None and sobj.width == 1 and sobj.height == 1 and  (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'bottomRight')  and not (hasattr(sobj, 'modifiedForSize')  and sobj.modifiedForSize&0x08 == 0x8):
                                        obj.tileset = sobj.tileset
                                        obj.type = sobj.original_type
                                        obj.original_type = sobj.original_type
                                        obj.modifiedForSize |= 0x08
                    if qp_data['top']['i'] is not None and QuickPaintOperations.countUp(layer, obj, qp_data['top']['oh']):
                        for y in range(qp_data['top']['oh']):
                            if y > 0:
                                sobj = QuickPaintOperations.searchObj(layer, obj.objx, obj.objy-y)
                                if sobj is not None and sobj.width == 1 and sobj.height == 1 and  (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'top') and not (hasattr(sobj, 'modifiedForSize')  and sobj.modifiedForSize&0x10 == 0x10):
                                    obj.tileset = sobj.tileset
                                    obj.type = sobj.original_type
                                    obj.original_type = sobj.original_type
                                    obj.modifiedForSize |= 0x10
                    if qp_data['bottom']['i'] is not None and QuickPaintOperations.countDown(layer, obj, qp_data['bottom']['oh']):
                        for y in range(qp_data['bottom']['oh']):
                            if y > 0:
                                sobj = QuickPaintOperations.searchObj(layer, obj.objx, obj.objy+y)
                                if sobj is not None and sobj.width == 1 and sobj.height == 1  and  (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'bottom') and not (hasattr(sobj, 'modifiedForSize')  and sobj.modifiedForSize&0x20 == 0x20):
                                    obj.tileset = sobj.tileset
                                    obj.type = sobj.original_type
                                    obj.original_type = sobj.original_type
                                    obj.modifiedForSize |= 0x20
                    if qp_data['left']['i'] is not None and QuickPaintOperations.countLeft(layer, obj, qp_data['left']['ow']):
                        for x in range(qp_data['left']['ow']):
                            if x > 0:
                                sobj = QuickPaintOperations.searchObj(layer, obj.objx-x, obj.objy)
                                if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'left')  and not (hasattr(sobj, 'modifiedForSize')  and sobj.modifiedForSize&0x40 == 0x40):
                                    obj.tileset = sobj.tileset
                                    obj.type = sobj.original_type
                                    obj.original_type = sobj.original_type
                                    obj.modifiedForSize |= 0x40
                    if qp_data['right']['i'] is not None and QuickPaintOperations.countRight(layer, obj, qp_data['right']['ow']):
                        for x in range(qp_data['right']['ow']):
                            if x > 0:
                                sobj = QuickPaintOperations.searchObj(layer, obj.objx+x, obj.objy)
                                if sobj is not None and sobj.width == 1 and sobj.height == 1 and (hasattr(sobj, 'autoTileType') and sobj.autoTileType == 'right') and not (hasattr(sobj, 'modifiedForSize')  and sobj.modifiedForSize&0x80 == 0x80):
                                    obj.tileset = sobj.tileset
                                    obj.type = sobj.original_type
                                    obj.original_type = sobj.original_type
