...
class SARC_Archive(FileArchive):
    ...
    def save(self):
        ...
        begOfDat = 0x20 + SFATNodesTableLen + 0x08 + len(fileNamesTable)

        # Create the File Data table
        fileDataTable = b''
        for file in files:
            
            # Pad it to 0x04, relative to file start
            totalFileLen = begOfDat + len(fileDataTable)
            if (totalFileLen % 0x04) != 0:
                fileDataTable += b'\x00' * (0x04 - (totalFileLen % 0x04))
        ...
        fileData = sarcHead + sfatHead + sfat + sfntHead + fileNamesTable + fileDataTable
